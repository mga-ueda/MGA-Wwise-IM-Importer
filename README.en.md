...
The workflow assumes **the Interactive Music data in Wwise is the master**. Keep one-shot and loop material on a single master waveform instead of pre-separating files; build structure nondestructively. For Wave-only / multi-wave, EXPORT copies the source WAV into Originals and maps ranges with MusicClip trim. For an XML long master, it splits the waveform per song (each contiguous run separated by `-R`). Gains and fades are not baked into the source WAV.
