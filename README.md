...
**音量・フェード・マーカー類は全面的に非破壊です。** ソース WAV も分割 WAV もゲインやフェードを焼き込まず、EXPORT 時は Wwise 側のプロパティ（MusicClip Fade／MusicFade／Make-Up Gain／Cue など）へ設定します。書き出し WAV は範囲の切り出しだけです。
...
左から: プロジェクト名コンボ → 書き出しパス → フォルダ／削除 → `Keep Last Session` → `Always on Top` → JP／EN → 歯車 → スペクトラム。
...
| Z | 波形表示エリアの高さを 1倍 → 2倍 → 3倍 → 1倍で循環（`[App] WaveformHeightScale`。文字入力フォーカス中は無効） |
...
左から: 位置表示 → TRANS（再生／G）→ NAV → TIME → AMP → SIZE（波形高さ Z）。
...
| SIZE（Z） | 波形表示エリアの高さを 1倍 → 2倍 → 3倍 → 1倍で循環（`[App] WaveformHeightScale`） |
...
| Ctrl+← / Ctrl+→ | 再生位置を前／次の Music Playlist 先頭・末尾、またはマーカーへ（グループ同期先の半透明マーカー含む。Wave 単体モードでは前後マーカー。← は押し続けている間は再生を止め、離すと再開） |
...
| Layer Music Option | More Options 上段中央。`Keep Layer Balance`（既定オフ）。Wwise の Loudness Normalization 利用時にオン推奨。EXPORT 時にグループ内各パートの LKFS を計測し、最も大きいパートの **Make-Up Gain を 0 dB**、それ以外は相対差だけ下げて **Music Track** へ**非破壊で**設定する（WAV へは焼き込まない）。未グループには適用しない |
...
- **マーカー** … Nuendo／Cubase 単発マーカーを行表示。Entry／Exit Cue より手前に描画する。グループ化された Playlist は、最も若い番号の Playlist を基準に、先頭からの相対位置とコメントを全メンバーで共有する。同期元は通常表示、同期先へ投影されたマーカーは三角だけを 25% の半透明で表示し、コメントは重複表示しない。Ctrl+←／→ の停止点にも含める。共有結果はプレビュー表示・Next Cue・Wwise Custom Cue に共通して使う（WAV へは埋め込まない）。途中からグループ化した場合も同じ規則で即時同期し、グループ解除時は各 Playlist 固有の元情報へ戻す
...
- **リージョン端フェード** … 白三角を内側へドラッグして非破壊の端フェード（プレビュー表示・再生。ソース WAV は変更しない）。ハンドルから真下へ 1px 白線。フェード範囲を右クリックでカーブ選択（名前・並びは Wwise MusicClip の CurveIn／CurveOut に合わせる。アウト側メニューは立ち下がりアイコン）。**Fade In は固まり先頭 Music Segment 内、Fade Out は末尾 Music Segment 内に制限**（Music Segment 同士はまたげない。`-A`／`-E` は隣接リージョンと同一セグメントのためその範囲もフェード可能）。**EXPORT 時は MusicClip の Fade Duration／Shape を非破壊で設定**（WAV へは焼き込まない）。WAAPI は Duration を 3.6 秒までしか受け付けないため、超過分はプロジェクトを一時クローズして WWU を直接編集する。Playlist 遷移フェードとは別物でゲインは重ねがけ。同一波形の Reload／再ドロップ時は Last Session サイドカーから復元。Ctrl+Z／Ctrl+Y で Undo／Redo
...
- **書き出し WAV は音声の切り出しのみ（非破壊）。** リージョン端フェード・レイヤーバランス用ゲイン・マーカー／Cue は WAV へ焼き込まず、続く Wwise 登録でインメモリ情報からプロパティへ設定する
...
**Wwise への載せ方も非破壊です。** 分割 WAV は切り出し音声のまま取り込み、フェード・レイヤーバランス・Cue・トランジションは Authoring 側プロパティへ設定します（ソース／分割 WAV は書き換えない）。

- グループ化されていない出力パート 1 つ、またはグループ 1 つを Music Playlist Container 1 つとして扱う
- 最終 Playlist が 2 つ以上なら Music Switch Container（元ファイル名）の下に配置する。あわせて **Music Switch 用 State Group**（同名）を `\States\Default Work Unit` に作成し、各 State を同名 Playlist に割当。既存 State Group があるときは削除・再作成せず、同一オブジェクトの State 一覧を現在の Playlist 構成へ更新する
- **グループ化 Playlist と State Group** … 2 パート以上のグループは、1 つの Music Segment 内へ複数 Music Track を置く縦レイヤーとして生成する。Playlist／Segment 名はグループ化後の連番へ詰める。あわせて次を行う:
  - **グループ名の State Group**を `\States\Default Work Unit` に作成し、グループ内 Playlist 数ぶんの State（`A` `B` `C` …）を置く
  - 各 Music Track の States へその State Group を割当し、**Track Volume** をレイヤー対応で設定する（1 番目 Track は A=0 dB／他 State=-108 dB、2 番目は B=0 dB／他=-108 dB …）
  - **Group Fade** … メンバー全員の秒数が同一なら **Default Transition Time のみ**（Custom TransitionList は書かない／既存はクリア）。秒数が分かれる場合のみ **Custom Transition Time**（総当たり。From→To の Time は遷移先 To）を書き、このときの Default Transition Time は Wwise 既定（1 秒）のまま（フォールバック最大値にはしない）
  - Switch 側の Any → Object ルールには、グループ代表パート（最小番号）の通常 Fade In／Out／Exit Source At を載せる（Group Fade 自体は Switch ルールへは書かない）
- **Authoring プレビュー用の現在 State** … インポート完了後（WWU パッチ〜プロジェクト再オープンの後）に `ak.soundengine.setState` で次を設定する。失敗してもインポートは続行し、ログに残す:
...
- **Layer Music Option / Keep Layer Balance（任意・既定オフ）** … Wwise の Loudness Normalization 利用時にオン推奨。EXPORT 時にグループ（2 パート以上）内の Integrated Loudness（LKFS）を計測し、最も大きいパートの Make-Up Gain を 0 dB、それ以外は相対差だけ下げて **Music Track** の Make-Up Gain へ**非破壊で**設定する。未グループには適用しない。WAV へは焼き込まない
...
| `MgaWwiseIMImporter.lastwave.<プロジェクト名>.json` | 追加マーカー位置、Playlist グループ／無効化、パート別 Fade In／Out／Group Fade・Exit Source At など |
...
| セクション | 内容 |
|------------|------|
| `[App]` | アプリ全体設定（Always on Top／表示言語／波形高さ倍率 など） |
...
| `WaveformHeightScale` | 波形表示エリアの高さ倍率（`1` / `2` / `3`）。Z キーで循環 | `1` |
| `DefaultWaveformFadeInCurve` / `DefaultWaveformFadeOutCurve` | 波形リージョン端フェードの既定カーブ（歯車メニュー）。既定はともに `SCurve` | `SCurve` |
| `DefaultPlaylistFadeInCurve` / `DefaultPlaylistFadeOutCurve` | Playlist 遷移フェードの既定カーブ（歯車メニュー）。既定はともに `SCurve` | `SCurve` |
...
| `LoudnessPreserveGroupBalance` | Layer Music Option / Keep Layer Balance（`1`/`0`）。グループ内相対バランスを Music Track の Make-Up Gain で非破壊維持 | `0` |
