...
/// 判定は埋め込みメタ（サステインループ smpl / リージョン / それ以外）のみ。
/// Wave 単体／複数波形で共通利用する。
/// </summary>
internal enum WaveOnlyMode
{
    ...
    /// <summary>サステインループ（smpl）あり。</summary>
    SmplLoop,
...
internal static class WaveOnlyModeProcessor
{
    ...
    /// <summary>
    /// サステインループ（smpl）の Start／End と同タイミングのため破棄した埋め込みマーカー 1 件。
    /// </summary>
    ...
    /// <summary>
    /// smpl ループ由来の <c>-L</c>/<c>-E</c> と、それ以外の埋め込みマーカー、
    /// および同タイミングで破棄したマーカーの一覧。
    /// </summary>
    ...
    /// <summary>
    /// モード B: サステインループ（smpl）の Start / End を <c>-L</c> / <c>-E</c> として採用する。
    /// 通常の埋め込みマーカー（コメント付き）も併せて読み取るが、
    /// サステインループの Start／End と同タイミングのマーカーは破棄する（サステイン優先）。
    /// Wave 単体／複数波形で共通。
    /// </summary>
        ...
        long frameCount)
    {
        ...
        var accepted = 0;
        var skipped = 0;
        var sustainSampleOffsets = new HashSet<long>();

        if (frameCount <= 0)
        {
            return new SmplLoopBuildResult(
                [],
                0,
                embedded.SmplLoops.Count,
                discarded);
        }
        ...
        foreach (var loop in embedded.SmplLoops)
        {
            ...
            markers.Add(new WaveformMarkerMark(
                ...
                IsFromWaveEmbedded: true));
            sustainSampleOffsets.Add(start);
            sustainSampleOffsets.Add(end);
            accepted++;
        }
        ...
        foreach (var point in embedded.PointMarkers)
        {
            ...
            if (sustainSampleOffsets.Contains(point.SampleOffset))
            {
    ...
    /// <summary>
    /// サステインループと同タイミングで破棄した埋め込みマーカーをログへ追記する。
    /// </summary>
